...
@Component({
  selector: 'app-act04',
  templateUrl: './act04.component.html',
  styleUrls: ['./act04.component.css']
})
export class Act04Component implements OnInit {

  idPaciente: string;
  idActividad = '5ed94657242c18b9df8e8654';
  actividad: Actividad;
  pacientes: Paciente[];
  imagenes: Imagen[];
  categorias = [];
  ...
  goActividad() {
    ...
    if (this.imgSelected.length < 6) {

      this.mensaje = 'Esta actividad <b> requiere un mínimo de 6 imágenes </b> para comenzar.'
  ...
  enviarTarea() {
    ...
    if (this.imgSelected.length < 6) {
      ...
      this.mensaje = 'Esta actividad <b> requiere un mínimo de 6 imágenes </b> para enviar.'
  ...
  ventanaInfo() {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res == true && this.permisoActividad) {
        this.router.navigate(['/actividades/actividad-04-play'], { skipLocationChange: true });
      }
